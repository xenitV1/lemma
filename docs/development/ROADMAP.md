...
- **B6** **Code-evidence + `snippet_hash` stale-detection (from [community PR #1](https://github.com/xenitV1/lemma/pull/1)).** A code-backed fragment may cite a file + symbol + a SHA-256 of the referenced snippet, captured as an **optional `evidence` field on `memory_add`** (no new tool). At recall time a *light, opt-in* check re-hashes the cited path; on mismatch the fragment is flagged stale (reuse B2's `invalidated_at` / `memory_validity` mechanism) and a resolution suggestion is surfaced — never hard-deleted. Fits the vision exactly: **LLM-free, pure-SQLite, Windows-safe, additive.** Deliberately *excludes* the PR's embeddings/vector core (see "Explicitly rejected"). **[compat: migration]** — additive `memory_evidence` table + optional param; recall check gated behind a `verification.stale_check` config key defaulting off until validated. Pairs with B2/B4/N9.

...
export const TOOL_NUDGES: Record<string, string> = {
  ...
  memory_add: "⚠️ Save new knowledge IMMEDIATELY — unsaved knowledge is lost forever. Never ask permission, just save. Worth saving = durable & reusable (a decision, gotcha, or pattern); skip one-off trivia. This reminder stops mattering once you've saved this task's insight.",
  memory_update: "When reality contradicts a memory, trust reality and update it here.",
  memory_feedback: "Give 👍 after a useful memory to boost its ranking; 👎 when it was wrong so it decays.",
  memory_forget: "Prune obsolete fragments. Prefer consolidate=true to archive (reversible) over hard delete.",
  memory_merge: "Fold duplicate/overlapping fragments into one. consolidate=true keeps the sources (superseded, reversible).",
  memory_relate: "Link fragments you notice are connected — supports / contradicts / supersedes / related_to (reverse auto-created).",
  ...
  conflict_scan: "Scan for contradicting memories; resolve high-confidence ones with memory_relate supersedes.",
  proactive_analysis: "Periodic health sweep — stale, orphan, low-quality, and distill signals for the whole base.",
  semantic_search: "Concept-level search when keyword memory_read misses paraphrases of the same idea.",
